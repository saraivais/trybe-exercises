...
function changeString(newString) {
  ...
  const where = exploded.indexOf('x');
  exploded[where] = newString;
  return exploded.join(' ');
}

console.log(changeString('xablau'));
...
const skills = ['HTML', 'CSS', 'JS', 'FlexBox', 'Python'];

function funcao2(string, array) {
  const skills = array.sort().join(', ')
  return `${string}! Minhas cinco principais habilidades são: ${skills} #goTrybe`;
}

console.log(funcao2(changeString('xablau'), skills));